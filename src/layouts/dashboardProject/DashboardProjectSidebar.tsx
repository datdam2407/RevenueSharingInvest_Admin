...
export default function DashboardProjectSidebar({
  ...
  const { isCollapse, collapseClick, collapseHover, onToggleCollapse, onHoverEnter, onHoverLeave } =
    useCollapseDrawer();
  // const { projectDetailBYID: businessProjectDetail, isLoading } = useSelector(
      ...
      >
        ...
        {isCollapse ? (
          ...
          <Link underline="none" component={RouterLink} to={PATH_DASHBOARD_PROJECT.project.root}>
            <AccountStyle>
              <Avatar src={project?.image ?? ''} alt={''}></Avatar>
              <Box sx={{ ml: 2 }}>
      ...
      {!isCollapse && (
        ...
        >
          <Typography gutterBottom variant="subtitle1">
            <Link href={PATH_DASHBOARD.projects.projectKrowd}>Quay lại bảng dự án</Link>
          </Typography>
        </Stack>
      )}
    </Scrollbar>
